...
import (
	...
	"ahava/pkg/config"
	"ahava/pkg/domain"
	"ahava/pkg/mock/mockhelper"
	"ahava/pkg/mock/mockrepo"
...
func Test_GetMyReferenceLink(t *testing.T) {
	...
	testData := map[string]struct {
		...
		"success": {
			...
			expectedOutput: "ahava.com/users/signup?ref=1234",
			expectedError:  nil,
		},
		...
		"success": {
			...
			StubDetails: func(userRepo mockrepo.MockUserRepository, orderRepo mockrepo.MockOrderRepository, helper mockhelper.MockHelper, data int) {
				gomock.InOrder(
					...
					userRepo.EXPECT().FindProductImage(1).Times(1).Return("image", nil),
					userRepo.EXPECT().FindProductImage(2).Times(1).Return("image", nil),
					userRepo.EXPECT().FindProductImage(3).Times(1).Return("image", nil),
